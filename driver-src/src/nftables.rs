...
impl NftGuard {
    pub fn new(all: bool) -> NftGuard {
        // 如果没添加ip规则，就添加
        if run_command("ip rule list fwmark 84").stdout.len() == 0 {
            run_command("ip rule add fwmark 84 lookup 84");
            println!("已配置ip rule规则")
        }
        if run_command("ip route list table 84").stdout.len() == 0 {
...
fn clear_nft() {
    run_command("nft delete table ip exp4");
}
...
pub fn clear_all() {
    clear_nft();
    // 清除ip配置
    while run_command("ip rule list fwmark 84").stdout.len() > 0 {
        run_command("ip rule delete fwmark 84");
    }
    run_command("ip route flush table 84");
